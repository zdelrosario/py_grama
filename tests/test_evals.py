...
class TestEvalInvariants(unittest.TestCase):
    ...
    def md_arg(self, func, df_arg="df", **kwargs):
        ...
        for wrong in self.type_tests:
            self.assertRaises(
                TypeError, func, wrong, **{df_arg: self.df}, **kwargs
            )

        ## No model.functions
        self.assertRaises(
            ValueError, func, gr.Model(), **{df_arg: self.df}, **kwargs
        )

    def df_arg(
        self, func, df_arg="df", shortcut=False, acc_none="Never", **kwargs
    ):
        ...
        for wrong in self.type_tests:
            self.assertRaises(
                TypeError, func, self.md, **{df_arg: wrong}, **kwargs
            )
        ...
        if shortcut:
            ...
            self.assertRaises(
                ValueError, func, self.md, **{df_arg: "a"}, **kwargs
            )
        else:
            # test any str for typeerror
            self.assertRaises(
                TypeError, func, self.md, **{df_arg: "nom"}, **kwargs
            )
        ...
        if acc_none == "var_det" or acc_none == "never":
            ...
            self.assertRaises(
                TypeError, func, self.md_var_det, **{df_arg: None}, **kwargs
            )
        if acc_none == "never":
            # none not accepted under any condition, test when md.var_det==0
            self.assertRaises(
                TypeError, func, self.md, **{df_arg: None}, **kwargs
            )
...
class TestDefaults(unittest.TestCase):
    ...
    def test_nominal(self):
        ...
        self.inv_test.df_arg(
            gr.eval_nominal, df_arg="df_det", shortcut=True, acc_none="var_det"
        )
    ...
    def test_linup(self):
        ...
        model_3d = (
            ...
            >> gr.cp_copula_gaussian(
                df_corr=gr.df_make(var1=["x1"], var2=["x2"], corr=[0.5])
            )
        )
        ...
        model_75 = (
            ...
            >> gr.cp_copula_gaussian(
                df_corr=gr.df_make(var1=["x1"], var2=["x2"], corr=[0.75])
            )
        )
        ...
        df_3d_outer = gr.eval_linup(
            model_3d, df_base=gr.df_make(x0=0, x1=0, x2=[0, 1])
        )
        self.assertTrue(df_3d_outer.shape[0] == 2 * 2)

        ## Test that sensitivity contributions sum to 1
        df_3d_nom = gr.eval_linup(
            model_3d, df_base=gr.df_make(x0=1, x1=0, x2=0), decomp=True
        )
        df_sum = (
            df_3d_nom
            >> gr.tf_group_by("out")
    ...
    def test_grad_fd(self):
        ...
        df_rand = gr.eval_grad_fd(
            md_test, df_base=df_base, var="rand", append=False
        )
        self.assertTrue(gr.df_equal(df_true[["Dy0_Dx0"]], df_rand, close=True))

        df_det = gr.eval_grad_fd(
            md_test, df_base=df_base, var="det", append=False
        )
        self.assertTrue(gr.df_equal(df_true[["Dy0_Dx1"]], df_det, close=True))
        ## Append base points
        df_append = gr.eval_grad_fd(
            md_test, df_base=df_base, var="det", append=True
        )
...
class TestRandomSampling(unittest.TestCase):
    ...
    def test_lhs(self):
        ...
        ## Pass-through
        df_pass = gr.eval_lhs(
            self.md_2d, n=n, skip=True, df_det="nom", seed=101
        )
    ...
    def test_sample(self):
        ...
        ## Pass-through
        df_pass = gr.eval_sample(
            self.md, n=n, skip=True, df_det="nom", seed=101
        )
...
class TestRandom(unittest.TestCase):
    ...
    def test_sample(self):
        ...
        self.inv_test.df_arg(
            gr.eval_sample, df_arg="df_det", shortcut=True, acc_none="var_det"
        )
