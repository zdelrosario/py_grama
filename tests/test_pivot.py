...
class TestPivotLonger(unittest.TestCase):
    ...
    def test_pivot_longer(self):
        ...
        long = gr.tran_pivot_longer(
            wide, columns=("One", "Two"), names_to="columns", values_to="values"
        )
        expected = gr.df_make(
    ...
    def test_pivot_longer_no_representation_index(self):
        ...
        wide = gr.df_make(A=[1, 2, 3], B=[4, 5, 6])
        long = gr.tran_pivot_longer(
            wide, columns=("A", "B"), names_to="var", values_to="value"
        )
        expected = DataFrame(
            {"var": ["A", "A", "A", "B", "B", "B"], "value": [1, 2, 3, 4, 5, 6]}
        )
    ...
    def test_pivot_longer_matches(self):
        ...
        stang = data.df_stang_wide
        long = gr.tran_pivot_longer(
            stang, columns=gr.matches("\\d+"), names_to="var", values_to="val"
        )
...
class TestPivotWider(unittest.TestCase):
    ...
    def test_pivot_wider(self):
        ...
        wide = gr.tran_pivot_wider(
            long, indexes_from="loo", names_from="columns", values_from="values"
        )
        expected = DataFrame(
            {
                "One": {"A": 1.0, "B": 2.0, "C": 3.0},
    ...
    def test_pivot_wider_duplicate_entries(self):
        ...
        with self.assertRaises(ValueError):
            ...
            wide = gr.tran_pivot_wider(
                long, names_from="var", values_from="val"
            )
    ...
    def test_pivot_wider_representation_index(self):
        ...
        wide = gr.tran_pivot_wider(long, names_from="var", values_from="val")
        expected = gr.df_make(
    ...
    def test_pivot_wider_NaN_entries(self):
        ...
        long = gr.tran_pivot_longer(
            original, columns=("A", "B"), names_to="var", values_to="value"
        )
        wide = gr.tran_pivot_wider(long, names_from="var", values_from="value")
        expected = gr.df_make(
            A=[1, 2, 3, NaN, NaN, NaN], B=[NaN, NaN, NaN, 4, 5, 6]
        )
