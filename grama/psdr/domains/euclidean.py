...
class EuclideanDomain(Domain):
    ...
    def _init_names(self, names):
        ...
        if isinstance(names, str):
            if len(self) == 1:
                ...
                self._names = [
                    names + " %d" % (j + 1,) for j in range(len(self))
                ]
        else:
            assert len(self) == len(
                names
            ), "Number of names must match dimension"
            self._names = names
    ...
    def isnormalized(self):
        return np.all(
            (~np.isfinite(self.norm_lb)) | (self.norm_lb == -1.0)
        ) and np.all((~np.isfinite(self.norm_ub)) | (self.norm_ub == 1.0))
    ...
    def _normalize_der(self):
        ...
        I = (
            (self.norm_ub != self.norm_lb)
            & np.isfinite(self.norm_lb)
            & np.isfinite(self.norm_ub)
        )
    ...
    def _isinside_ineq(self, X, tol=None):
        if tol is None:
            tol = self.tol
        return np.array(
            [np.all(np.dot(self.A, x) <= self.b + tol) for x in X],
            dtype=np.bool,
        )
    ...
    def _isinside_eq(self, X, tol=None):
        if tol is None:
            tol = self.tol
        return np.array(
            [np.all(np.abs(np.dot(self.A_eq, x) - self.b_eq) < tol) for x in X],
            dtype=np.bool,
        )
