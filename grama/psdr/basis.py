...
class PolynomialTensorBasis(Basis):
    ...
    def V(self, X=None):
        ...
        M = X.shape[0]
        assert X.shape[1] == self.dim, "Expected %d dimensions, got %d" % (
            self.dim,
            X.shape[1],
        )
        V_coordinate = [
            self.vander(X[:, k], self.degree) for k in range(self.dim)
        ]
    ...
    def roots(self, coef):
        ...
        return (
            r * (self._ub[0] - self._lb[0]) / 2.0
            + (self._ub[0] + self._lb[0]) / 2.0
        )
...
class ArnoldiPolynomialBasis(Basis):
    ...
    def _update_vec(self, ids):
        ...
        j = np.max(
            np.argwhere(
                (np.sum(np.abs(diff), axis=1) <= 1)
                & (np.min(diff, axis=1) == -1)
            )
        )
