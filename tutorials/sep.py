...
for id_worksheet in range(len(nb_orig["worksheets"])):
    ...
    for id_cell in range(len(worksheet_orig["cells"])):
        ...
        if cell_orig["cell_type"] == "markdown":
            ...
            raise ValueError(
                "Unrecognized cell type {}".format(cell_orig["cell_type"])
            )
        ...
        assignment_text = re.sub("# task-(begin|end)\n+", "", assignment_text)
        ...
        solution_text = re.sub("# solution-(begin|end)\n+", "", solution_text)
        ...
        solution_text = re.sub(
            "# task-begin(\n|.)*?# task-end", "", solution_text
        )

        ## Write the results
        ## Switch based on cell type
        if cell_orig["cell_type"] == "markdown":
            nb_assignment["worksheets"][id_worksheet]["cells"][id_cell][
                "source"
            ] = assignment_text
            nb_solution["worksheets"][id_worksheet]["cells"][id_cell][
                "source"
            ] = solution_text
        elif cell_orig["cell_type"] == "code":
            nb_assignment["worksheets"][id_worksheet]["cells"][id_cell][
...
## Output
nbformat.write(nb_assignment, filename_assignment, version=4)

nbformat.write(nb_solution, filename_solution, version=4)
